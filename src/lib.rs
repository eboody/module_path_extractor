...
pub fn get_source_info() -> Option<(String, usize)> {
    ...
    let line_number = span.start().line(); // Get the line number
...
pub fn find_module_path(file_path: &str, line_number: usize) -> Option<String> {
    let content = fs::read_to_string(file_path).ok()?; // Read file contents
    ...
    if !module_path.is_empty() {
        return Some(module_path.join("::").to_string());
    }
